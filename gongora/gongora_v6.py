...
"""
════════════════════════════════════════════════════════════════════════════════
  gongora_v6.py — Atribución prosódica flexible
  ...
  Uso:
    python3 gongora_v6.py escansion.csv --ref A+B+S --eval D
    python3 gongora_v6.py escansion.csv --ref A --eval B,D --also B
    python3 gongora_v6.py escansion.csv --ref A+B --eval S,D --also A,B
    python3 gongora_v6.py escansion.csv --ref B+S --eval A,D --also B,S
  ...
  Opciones:
    ...
    --html  Fichero HTML de salida (default: gongora_v6.html)
    --json  Fichero JSON de salida (default: gongora_v6.json)

  Vector: 31 rasgos prosódicos (alineado con gongora_attribution_v52.py)
...
import csv, math, json, random, argparse, sys
...
# ══════════════════════════════════════════════════════════════════════════════
# RASGOS (31 dimensiones, alineado con v5.2)
# ══════════════════════════════════════════════════════════════════════════════
...
def extract_features(sonnet):
    """Vector de 31 rasgos prosódicos."""
    ...
    n_verses = len(ent_vals)
    pct_exclam = sum(1 for e in ent_vals if e == 1) / n_verses if n_verses else 0
    pct_interrog = sum(1 for e in ent_vals if e == 2) / n_verses if n_verses else 0

    first_mark = None
    for i, e in enumerate(ent_vals):
        if e > 0:
            first_mark = i; break
    pos_first = (first_mark / (n_verses - 1)) if (first_mark is not None and n_verses > 1) else 0.5
...
<title>Góngora v6 — Atribución prosódica interactiva</title>
...
<h1>Atribución prosódica v6 &mdash; Sonetos de Góngora</h1>
<div class="sub">31 rasgos prosódicos &middot; Mahalanobis &middot; PCA &middot; Selección interactiva de corpus</div>
...
def main():
    parser = argparse.ArgumentParser(
        description="Gongora v6 — Atribución prosódica flexible",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ejemplos:
  python3 gongora_v6.py esc.csv --ref A+B+S --eval D
  python3 gongora_v6.py esc.csv --ref A --eval B,D --also B
  python3 gongora_v6.py esc.csv --ref A+B --eval S,D --also A,B
  python3 gongora_v6.py esc.csv --ref B+S --eval A,D
        """
    )
    ...
    parser.add_argument('--html', default='gongora_v6.html')
    parser.add_argument('--json', default='gongora_v6.json')
    ...
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  Góngora v6 — Atribución prosódica flexible               ║")
    print("║  31 rasgos · Mahalanobis · PCA                            ║")
    print("╚══════════════════════════════════════════════════════════════╝")
